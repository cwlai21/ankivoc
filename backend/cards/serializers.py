...
class VocabularyBatchCreateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        # Check that target language has a card template
        if not hasattr(data['target_language'], 'card_template'):
            raise serializers.ValidationError(
                f'No card template configured for {data["target_language"].name}. '
